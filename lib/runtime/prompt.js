...
import { getCurrentRuntime, WorkflowPausedError } from './runtime.js';
...
export async function initialPrompt(question, options = {}) {
  ...
  // Check memory for already answered prompt
  if (runtime._rawMemory[memoryKey] !== undefined) {
    console.log(`  [Prompt: ${slug}] Using saved response`);
    return runtime._rawMemory[memoryKey];
  }
