...
export function startServer(workflowDir, initialPort = 3000) {
  ...
  const requestHandler = (req, res) => {
    ...
    // Submit Answer (Local Interactivity)
    if (req.url === '/api/submit' && req.method === 'POST') {
      let body = '';
      req.on('data', chunk => body += chunk.toString());
      req.on('end', () => {
        try {
          const { slug, response } = JSON.parse(body);
          if (!slug || !response) {
            res.writeHead(400);
            res.end(JSON.stringify({ error: 'Missing slug or response' }));
            return;
          }

          const interactionsDir = path.join(workflowDir, 'interactions');
          if (!fs.existsSync(interactionsDir)) {
            fs.mkdirSync(interactionsDir, { recursive: true });
          }

          const interactionFile = path.join(interactionsDir, `${slug}.md`);
          fs.writeFileSync(interactionFile, response);
